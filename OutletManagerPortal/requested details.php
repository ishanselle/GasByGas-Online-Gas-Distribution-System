...
<body>
			<nav>
				<a href="sendmail.php"><i class="fas fa-envelope"></i> Email</a>
				<a href="give token.php"><i class="fas fa-coins"></i>  Tokens</a>
			</nav>
...
/* Side Bar Options */
 ...
 /* Navigation Bar Styling */
        nav {
            ...
            padding: 10px 0;
            box-shadow: 0px 20px 10px rgba(247, 110, 235, 0.85);
            z-index: 1000;
			margin-top: 100px;
			margin-left: 300px;
			border-radius: 5px; 
 ...
 /* Detail View Table Styling */
    ...
    <header>
        ...
        ----Welcome to Requested Details Page ----
    </header>
     <aside id="aside">
  ...
  <?php
	...
	if (!empty($_GET['outlet_name']) && !empty($_GET['outlet_id']) && !empty($_GET['requested_date'])) {
		$outlet_name = $conn->real_escape_string($_GET['outlet_name']);
		$outlet_id = $conn->real_escape_string($_GET['outlet_id']);
		$requested_date = $conn->real_escape_string($_GET['requested_date']);

		// **SQL Query**
		$sql = "SELECT * FROM gas_request 
				WHERE Outlet_name LIKE '%$outlet_name%' 
				AND Outlet_id = '$outlet_id' 
				AND Gas_Req_day = '$requested_date'";
		...
		echo "<div class='no-records'>
				<marquee behavior='alternate' direction='right' scrolldelay='20' scrollamount='10'>
					Please fill in all three fields, find your relevent request.
			  </marquee>
			</div>";
	}
